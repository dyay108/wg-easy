...
AllowedIPs = ${allowedIps.join(', ')}${extraLines.length ? `\n${extraLines.join('\n')}` : ''}`;
  ...
  generateServerInterface: (
    ...
    // Split hooks into individual commands (one per line)
    const preUpLines = hooks.preUp.split('\n').filter(cmd => cmd.trim()).map(cmd => `PreUp = ${iptablesTemplate(cmd, wgInterface)}`);
    const postUpLines = hooks.postUp.split('\n').filter(cmd => cmd.trim()).map(cmd => `PostUp = ${iptablesTemplate(cmd, wgInterface)}`);
    const preDownLines = hooks.preDown.split('\n').filter(cmd => cmd.trim()).map(cmd => `PreDown = ${iptablesTemplate(cmd, wgInterface)}`);
    const postDownLines = hooks.postDown.split('\n').filter(cmd => cmd.trim()).map(cmd => `PostDown = ${iptablesTemplate(cmd, wgInterface)}`);
